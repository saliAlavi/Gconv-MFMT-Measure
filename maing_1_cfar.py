...
class NeuralNetwork(nn.Module):
    def __init__(self):
        ...
        for layer in range(len(channel_size)):
            ...
            p = int(k//2)
            if layer == 0:
                self.conv2d.extend([nn.Conv2d(3, oc, kernel_size=(k, k), stride=(s, s), padding=(p, p), bias=False)]) 
                self.p4convz2.extend([P4ConvZ2(in_channels=3, out_channels=oc, kernel_size=k, stride=s, padding=p)])
                self.p4mconvz2.extend([P4MConvZ2(in_channels=3, out_channels=oc, kernel_size=k, stride=s, padding=p)])
            else:
...
if __name__ == "__main__":
    fn_model = 'temp/model_g_1_cfar.pt'
    data_transformation = {
